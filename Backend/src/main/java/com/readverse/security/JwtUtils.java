...
import com.readverse.custom_exception.JwtProcessingException;
...
public class JwtUtils {
	...
	public String generateJwtToken(Authentication authentication) {
		...
		// contains verified user details
		log.info("generate jwt token " + authentication);

		User userPrincipal = (User) authentication.getPrincipal();
		return Jwts.builder() // JWTs : a Factory class , used to create JWT tokens

				// setting subject part of the token
				.subject((userPrincipal.getUsername()))

				// Sets the JWT Claims iat (issued at) value of current date
				.issuedAt(new Date()).expiration(new Date((new Date()).getTime() + jwtExpirationMs))// Sets the JWT
																									// Claims exp
																									// (expiration)
				.claim("id", userPrincipal.getId()) // value.
...
//					new UsernamePasswordAuthenticationToken(new User(email,"", authorities), null, authorities);
	...
	public Long extractUserIdFromToken(String authHeader) {
	    ...
	    try {
	        ...
	        throw new JwtProcessingException("Invalid or missing 'id' claim in token");
	    }
	}
